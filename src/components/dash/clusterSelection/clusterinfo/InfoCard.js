  ...
  if (!flip)
    return (
      <>
        ...
        >
          <div className="w-full flex flex-col items-center">
            <div className="p-4 pb-0 flex-1">
              <h1 className=" mb-1 text-center text-grey-darkest">
                {name === "rank" ? "Score" : name}
              </h1>
        ...
        >
          <LineChart arr={lineArr} dates={dates} name={name} />
        </div>
      </>
    );
  else
    ...
    return (
      <>
        ...
        <div style={{ gridArea: "b" }}>
          <h2 className="mb-4">{name === "rank" ? "Score" : name}</h2>
          <button
